...
class Carrinho {
	...
	protected function doVer() {
		...
		$carrinho = $this->system->sessionPortal->getItem('carrinho_produtos');
		$cupom = $this->system->sessionPortal->getItem('carrinho_cupom');
		$this->system->sessionPortal->deleteItem('carrinho_total');
		$produtos = array();
		$total = 0;
		...
		$this->system->sessionPortal->addItem('carrinho_total', $total);

		//msg erro
		$msg_error = $this->system->sessionPortal->getItem('msg_error');
		$this->system->sessionPortal->deleteItem('msg_error');
	...
	protected function doAdicionarCurso() {
		...
		$produtos = $this->system->sessionPortal->getItem('carrinho_produtos');
		$produtos[] = $produto;
		$this->system->sessionPortal->addItem('carrinho_produtos', $produtos);	
	...
	protected function doAdicionarCupom() {
		...
		$cupomCarrinho = $this->system->sessionPortal->getItem('carrinho_cupom');		
		...
		if ($msg_error) 
			$this->system->sessionPortal->addItem('msg_error', $msg_error);		
		else 
			$this->system->sessionPortal->addItem('carrinho_cupom', $cupom);		
		$this->redirecionarCarrinho();
	}
	// ==============================================================
	protected function doRemoverCupom() {
		$this->system->sessionPortal->deleteItem('carrinho_cupom');		
		$this->redirecionarCarrinho();		
	}
	// ==============================================================
	protected function doVerificaLogin() {
		...
		//msg sucesso
		if ($msg_success = $this->system->sessionPortal->getItem('msg_success')) {
			$this->system->sessionPortal->deleteItem('msg_success');			
			$this->system->view->assign('msg_success', $msg_success);	
		}
		...
		if ($enviar) {
			...
			if ($dados) {
				...
				//setcookie("cookie_cod_usuario", $dados->id, null, '/'); 
	...
	protected function doVerificaCompra() {
		...
		if (!$carrinho = $this->system->sessionPortal->getItem('carrinho_produtos')) {
		...
		$vendaID = $this->system->sessionPortal->getItem('venda_corrente');
		if ($vendaID) {
		...
		$this->system->sessionPortal->addItem('venda_corrente', $id);
		$this->system->sessionPortal->addItem('venda_numero', str_pad($id, 5, "0", STR_PAD_LEFT));
		...
		if ($this->system->sessionPortal->getItem('carrinho_total') == 0) {
	...
	protected function doPagamento() {
		$vendaID = $this->system->sessionPortal->getItem('venda_corrente');
		$vendaNumero = $this->system->sessionPortal->getItem('venda_numero');
		if (!$vendaID) {
			$this->system->sessionPortal->addItem('msg_error', 'Houve uma falha, tente novamente mais tarde');	
			$this->redirecionarCarrinho();		
		}

		$carrinho = $this->system->sessionPortal->getItem('carrinho_produtos');
		$cupom = $this->system->sessionPortal->getItem('carrinho_cupom');
		$total = $this->system->sessionPortal->getItem('carrinho_total');
		$produtos = array();
		$plano = false;
		foreach ($carrinho as $key => $item) {
			$produto = array();
	...
	protected function doConcluirPagSeguro() {
		...
		$valor = $this->system->sessionPortal->getItem('carrinho_total');
		$vendaNumero = $this->system->sessionPortal->getItem('venda_numero');
	...
	private function doConcluirGratuito() {
		$vendaID = $this->system->sessionPortal->getItem('venda_corrente');
		$numero = $this->system->sessionPortal->getItem('venda_numero');
		if ($vendaID) {
			$campos['status'] = 1;
	...
	protected function doAssinar() {
		...
		if ($enviar) {
			...
			if (count($erro) == 0) {
				...
				$this->system->pagseguro_model->setReferencia($this->system->sessionPortal->getItem('venda_corrente'));
				...
				try {
					...
					$preApprovalCode = $this->system->pagseguro_model->getPreApprovalCode();

					$this->system->vendas->atualizar($this->system->sessionPortal->getItem('venda_corrente'), array('forma_pagamento' => 1, 'codePagSeguro' => $preApprovalCode));
					$this->system->sessionPortal->deleteItem('venda_corrente');
					$this->system->sessionPortal->deleteItem('venda_numero');
					$this->system->sessionPortal->deleteItem('msg_error');
					$this->system->sessionPortal->deleteItem('carrinho_produtos');
					$this->system->sessionPortal->deleteItem('carrinho_cupom');
					$this->system->sessionPortal->deleteItem('carrinho_total');
					$this->system->sessionPortal->deleteItem('pagamento_assinatura_mensagem');
					$this->system->sessionPortal->deleteItem('pagamento_assinatura');
					$this->system->sessionPortal->deleteItem('pagamento_assinatura_id');
	...
	protected function doConfirmacao() {	
		...
		$assinatura = $this->system->sessionPortal->getItem('pagamento_assinatura');
		$assinaturaID = $this->system->sessionPortal->getItem('pagamento_assinatura_id');
		$assinaturaMensagem = $this->system->sessionPortal->getItem('pagamento_assinatura_mensagem');
	...
	protected function doCancelar() {
		...
		$this->system->sessionPortal->addItem('msg_error', 'Venda cancelada');	
		$this->redirecionarCarrinho();		
	}
	// ==============================================================
