	...
	protected function doCadastro() {
		...
		if ($editar) {
			...
			if ($erro_msg) {
				...
				if (count($this->system->sessionPortal->getItem('carrinho_produtos')) > 0)  {
					$this->system->sessionPortal->addItem('msg_success', 'Sua conta foi criada com sucesso, pode realizar o login e continuar sua compra.');
					session_write_close();
					header('location: ' . $this->system->getUrlSite() . 'carrinho/verificaLogin/' );
					exit();
				}
			}
		}
		...
		$this->system->view->display('site/cadastro.tpl');
