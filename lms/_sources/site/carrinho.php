...
class Carrinho {
	...
	public function autoRun() {
		switch($this->system->input['do']) {
			...
			case 'assinarPagSeguro':	$this->doAssinarPagSeguro(); break;
			case 'enviaAssinatura':		$this->doEnviaAssinatura(); break;			
				...
				setcookie("cookie_cod_usuario", $dados->id, 0, '/', '.cursosiag.com.br'); 
		       	...
		       	if ($this->system->planos->verificaAssinaturaAtiva(' and usuario_id = ' . $this->system->session->getItem('session_cod_usuario'))) {
		        	...
		        	//header('Location: ' . $this->system->getUrlSite() . 'carrinho/ver/');
     				echo "<script type='text/javascript'>window.location.href='" . $this->system->getUrlSite(). "carrinho/ver'</script>";
		        	exit();
		       	}
		        else {
		        	session_write_close();
	...
	protected function doVerificaCompra() {
		...
		$this->system->session->addItem('venda_numero_pagarme', $id);
		...
		if ($this->system->session->getItem('carrinho_total') == 0) {
			...
			echo "<script type='text/javascript'>window.location.href='" . $this->system->getUrlSite(). "carrinho/pagamento'</script>";
			exit();			
		}
	}
	// ==============================================================
	protected function doPagamento() {
		...
		foreach ($carrinho as $key => $item) {
			...
			if ($item['tipo'] == 'curso') {
				...
				$produto['tipo'] 		= 'curso';
				$produto['id'] 			= $item['id'];
				$detalhesProdutos .= "ID : ".$item['id'] . " - Curso : ".$item['curso'];
				if(strlen($detalhesProdutos) < 53){
					$res = 53 - strlen($detalhesProdutos);
					for ($i=0; $i < $res; $i++) { 
						$detalhesProdutos .= '-';	
					}
				}else{
					$res = 105 - strlen($detalhesProdutos);
					for ($i=0; $i < $res; $i++) { 
						$detalhesProdutos .= ' -';	
					}
				}
	...
	// ==============================================================
	protected function doEnviaAssinatura() {
		$id_assinatura_pagarme = $this->system->session->getItem('id_assinatura_pagarme');
		if ($id_assinatura_pagarme) {
			...
			$this->system->view->assign('total', number_format($total, 2, ',', '.'));
			$this->system->view->assign('totalPagarme', number_format($total, 2, '', ''));
			$this->system->view->assign('id_assinatura_pagarme', $id_assinatura_pagarme);
			$this->system->site->topo($this->tituloPagina);
			$this->system->view->display('site/enviaAssinatura.tpl');
			$this->system->site->rodape();		
		}
	}
	// ==============================================================
	...
	protected function doConcluirPagarme() {
		...
		if ($enviado) {
			...
			$email = $this->system->session->getItem('session_email');
			$nome = $this->system->session->getItem('session_nome');
			$detalhesProdutos = $this->system->session->getItem('detalhesProdutos');
			

			$this->system->pagamento_model->setValor($valor);
			$this->system->pagamento_model->setTransacaoID($vendaNumero);
			$this->system->pagamento_model->setDetalhesProdutos($detalhesProdutos);
			$this->system->pagamento_model->setEmailUsuario($email);
			$this->system->pagamento_model->setNomeUsuario($nome);
			$this->system->pagamento_model->setParcelas($parcelas);
			...
			if ($retorno['status']) {
				...
				if(!empty($retorno['conteudo']->boleto_url)){
					$this->system->session->addItem('boletoURL',$retorno['conteudo']->boleto_url);
					$this->system->session->addItem('boletoBarcode',$retorno['conteudo']->boleto_barcode);
				}
				...
				if ($vendaID) {
					$campos['forma_pagamento'] = 6;
					$campos['codePagarme'] 	   = $retorno['conteudo']->id;					
					$this->system->vendas->atualizar($vendaID, $campos);
					// echo "<script type='text/javascript'>window.location.href='" . $this->system->getUrlSite(). "carrinho/confirmacao'</script>";
				}	
			}
	...
	protected function doAssinar() {
		...
		if ($enviar) {
			...
			if (count($erro) == 0) {
				...
				$this->system->pagarme_model->setValor(number_format($plano['valor'],2,'',''));
				$this->system->pagarme_model->setPlano($plano['codigoPlanoPagarme']);
				$this->system->pagarme_model->setDetalhesProdutos($detalhesProdutos);
				...
				try {
					...
					if ($retorno['pago']) {						
						...
						$this->system->vendas->atualizar($this->system->session->getItem('venda_numero_pagarme'), array('forma_pagamento' => 6, 'codePagarme' => $id_transacao_pagarme));
						$this->system->session->addItem('id_assinatura_pagarme', $id_assinatura_pagarme);
												
						if(!empty($retorno['conteudo']->current_transaction->boleto_url)){
	...
	// =============================================================
	protected function doAssinarPagSeguro() {
		$enviar = $this->system->input['enviar'];
	...
	protected function doConfirmacao() {	
		...
		$this->system->session->deleteItem('boletoURL');
		$this->system->session->deleteItem('boletoBarcode');

		if(!empty($boletoURL)){
			$this->system->view->assign('boletoBarcode', $boletoBarcode);
			$this->system->view->assign('boletoURL', $boletoURL);	
		}
