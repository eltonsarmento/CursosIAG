...
class Curso {
	...
	protected function doVisualizar() {
		...
		if ($id) {
			...
			$curso['valor_12'] = number_format(($curso['valor'] / 12), 2, ',', '.');
			...
			//cursos relacionados
			$cursosRelacionados = $this->system->curso->getCursosRelacionados($curso['id']);
	...
	protected function doBuscar() {
		...
		$this->system->sessionPortal->deleteItem('resultado_busca_mais');
		...
		$this->system->sessionPortal->addItem('resultado_busca_mais', $cursosListarMais);
	...
	protected function doCarregarMais() {
		$cursos = $this->system->sessionPortal->getItem('resultado_busca_mais');
		$this->system->sessionPortal->deleteItem('resultado_busca_mais');
	...
	protected function doPreco() {
		...
		echo "jQuery('.12_vezes').html('ou até 12x de R$ " . number_format($preco/12, 2, ',', '.') . "');";
		echo "</script>";
	}
	// ===============================================================
