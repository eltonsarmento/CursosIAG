                ...
                <div class="row-fluid">
                    ...
                    <div class="span12">
                        ...
                        <div class="widgetbox">
                            ...
                            <h4 class="widgettitle">{$ultima_dica.titulo} <a class="close">&times;</a> <a class="minimize">&#8211;</a></h4>

                            <div class="widgetcontent">
                                {if $ultima_dica.tipo_video eq 1}
                                    <!-- YOUTUBE -->
                                    <iframe width="100%" height="578" src="{$ultima_dica.link_video}" frameborder="0" allowfullscreen></iframe>
                                {else}
                                    <!-- VIMEO -->
                                    <iframe src="{$ultima_dica.link_video}" width="100%" height="578" frameborder="0" webkitallowfullscreen mozallowfullscreen allowfullscreen></iframe>
                                {/if}
                                
                                <i>Fonte:</i> <strong>{$ultima_dica.fonte}</strong>
                            </div><!--widgetcontent-->
                            {else}

                            <h4 class="widgettitle">Não há dicas! <a class="close">&times;</a> <a class="minimize">&#8211;</a></h4>
                        ...
                        <div class="widgetcontent nopadding">
                            <ul class="commentlist">
                                {foreach from=$mais_dicas item=dicas}
                                <li {if $dicas.visualizado eq 1}style="background:#f0f0f0;"{else}style="margin-left:0;"{/if} id="dicas_{$dicas.id}" class="{if $dicas.visualizado eq 1}view {else} not-view {/if}">
                                    <div class="comment-info" style="margin-left:0;">
                                        <h4><strong><a href="#myModal" onclick="abrirMensagem({$dicas.id})" data-toggle="modal">{$dicas.titulo}</a></strong></h4>
                                        <p>{$dicas.descricao|html_entity_decode} </p>
                                    </div>
                                </li>
                                {/foreach}
