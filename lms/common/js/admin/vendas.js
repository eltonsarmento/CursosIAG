	...
	jQuery.ajax({
		...
		data: { mes: mes, ano: ano, professor: professor }}).done(
			function html(html) {
				...
				var tabela = '';
				var ValorTotalMes = 0;
                var vendasTotalAprovadas = 0;
				jQuery.each(data.ultimas_10_vendas_mes, function(i, item) {
					ValorTotalMes = ValorTotalMes + parseFloat(item.valor_total);
                    var dataPedido = item.data_venda.split(/[-]/);
					tabela += '<tr><td><a href="/lms/' + categoria + '/vendas/detalhes/' + item.id + '">' + item.numero + '</a></td><td>' + item.cliente.nome + '</td><td>' + dataPedido[2] + '/' + dataPedido[1] + '/' + dataPedido[0] + '</td><td>' + item.cursos + '</td><td class="center">R$ ' + item.valor_total + '</td></tr>';
                    vendasTotalAprovadas++;
				});
                ...
                jQuery('.vendas_aprovadas_total').html(vendasTotalAprovadas);
                jQuery('.vendas_mes_total').html(data.total_vendas_mes);
                jQuery('.valor_total_mes').html('R$ ' + parseFloat(ValorTotalMes).toFixed(2));
			}
		);
}
